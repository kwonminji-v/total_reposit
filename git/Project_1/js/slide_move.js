...
/* ㅡㅡㅡㅡ마지막 슬라이드에 도달했을 때 넘어가는 기능을 넣기위한 자바스크립트ㅡㅡㅡㅡ */
...
/* ㅡㅡㅡㅡ자동으로 넘어가는 기능을 넣기위한 자바스크립트ㅡㅡㅡㅡ */
 ...
 function autoSlide() {
     if(timer == undefined) {
         timer = setInterval(function(){
            ...
            //  page_btn.style.border="none";
            for ( let i = 1; i < currentIdx; i++) {
                page_btn[i].style.border="2px solid red";
                page_btn[i-1].style.borderColor="gray";
            }
        }, 1000);
    }
}
...
// 이미지 하단 라디오 버튼을 이용하여 움직이게하기.
let page_btn = document.querySelectorAll('.page_radio')
let page_btn_box = document.querySelector('.pagination_box')


    for (let i=0; i<slideCount; i++) {
        page_btn[i].addEventListener('click', function() {
            console.log(i)
            slides.style.transform = `translateX(-${i*1920}px)`
        })
        page_btn_box.addEventListener('mouseenter',function() {
            stopSlide();
        })
        page_btn_box.addEventListener('mouseleave',function() {
            autoSlide();
    })
}
pagebtn_move()